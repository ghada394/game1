...
export const npc_entity = (() => {
  ...
  class NPCController extends entity.Component {
    ...
    _FindPlayer(pos) {
      ...
      const playerPos = nearby[0].entity._position;

      // 🔒 تحقق هل اللاعب داخل البيت
      if (playerPos.x >= 30 && playerPos.x <= 50 &&
          playerPos.z >= 30 && playerPos.z <= 50) {
        // لا تتبع اللاعب (لا توجد حركة)
        return new THREE.Vector3(0, 0, 0);
      }
