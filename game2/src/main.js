...
class HackNSlashDemo {
  ...
  _SpawnMonstersIfNight() {
    ...
    for (let i = 0; i < 10; ++i) {
      ...
      npc.AddComponent(new health_component.HealthComponent({
        health: 50, maxHealth: 50, strength: 2, wisdomness: 2,
        benchpress: 3, curl: 1, experience: 0, level: 1,
        camera: this._camera, scene: this._scene,
      }));
      npc.AddComponent(new spatial_grid_controller.SpatialGridController({ grid: this._grid }));
      npc.AddComponent(new health_bar.HealthBar({ parent: this._scene, camera: this._camera }));
      npc.AddComponent(new attack_controller.AttackController({ timing: 0.35 }));
      npc.SetPosition(new THREE.Vector3((Math.random() * 2 - 1) * 500, 0, (Math.random() * 2 - 1) * 500));
  ...
  _Initialize() {
    ...
    const fov = 60, aspect = 1920 / 1080, near = 1.0, far = 10000.0;
    ...
    const plane = new THREE.Mesh(new THREE.PlaneGeometry(5000, 5000), new THREE.MeshStandardMaterial({ color: 0x1e601c }));
    ...
    forestWorld.AddComponent(new gltf_component.StaticModelComponent({
      ...
      scale: 100, receiveShadow: true, castShadow: true,
    }));
    ...
    forestHut.AddComponent(new gltf_component.StaticModelComponent({
      ...
      scale: 10, receiveShadow: true, castShadow: true,
    }));
  ...
  _LoadSky() {
    ...
    const skyMat = new THREE.ShaderMaterial({
      uniforms: uniforms, vertexShader: _VS, fragmentShader: _FS, side: THREE.BackSide
    });
  ...
  _LoadFoliage() {
    for (let i = 0; i < 5; ++i) {
      const names = ['CommonTree_Dead', 'CommonTree', 'BirchTree', 'BirchTree_Dead', 'Willow', 'Willow_Dead', 'PineTree'];
      const name = names[math.rand_int(0, names.length - 1)];
      const index = math.rand_int(1, 5);
      const pos = new THREE.Vector3((Math.random() * 2.0 - 1.0) * 500, 0, (Math.random() * 2.0 - 1.0) * 500);
      const e = new entity.Entity();
      e.AddComponent(new gltf_component.StaticModelComponent({
        scene: this._scene,
        resourcePath: './resources/nature/FBX/',
        resourceName: name + '_' + index + '.fbx',
        scale: 0.25, emissive: new THREE.Color(0x000000),
        specular: new THREE.Color(0x000000), receiveShadow: true, castShadow: true,
      }));
  ...
  _LoadHomeAndMerchant() {
    const updatePlayerGold = (amount) => {
      const playerEntity = this._entityManager.Get('player');
      if (!playerEntity) return;
      const inventory = playerEntity.GetComponent('InventoryController');
      if (inventory) inventory.AddGold(amount);
    };
    const addInventoryItem = (itemObj) => {
      const playerEntity = this._entityManager.Get('player');
      if (!playerEntity) return;
      const inventory = playerEntity.GetComponent('InventoryController');
      if (inventory) inventory.AddItem(itemObj);
    };
    const upgradeHome = () => {
      const homeEntity = this._entityManager.Get('home');
      if (homeEntity) homeEntity.GetComponent('HomeComponent').upgradeHome();
    };
    window.addGold50 = () => { updatePlayerGold(50); console.log('تمت الإضافة: 50 ذهب'); };
  ...
  _LoadPlayer() {
    ...
    player.AddComponent(new health_component.HealthComponent({
      updateUI: true, health: 100, maxHealth: 100, strength: 50, wisdomness: 5,
      benchpress: 20, curl: 100, experience: 0, level: 1,
    }));
    ...
    camera.AddComponent(new third_person_camera.ThirdPersonCamera({ camera: this._camera, target: this._entityManager.Get('player') }));
  ...
  _Step(timeElapsed) {
    ...
    this._hud.update({ health: playerHealth, day: this._dayCount, enemies: numEnemies, gold: currentPlayerGold });
  }
}
