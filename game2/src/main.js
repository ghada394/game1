...
class HackNSlashDemo {
  ...
  _LoadPlayer() {
    ...
    // START_CHANGE
    // Removed initial axe and sword creation as they are now bought from the merchant
...
// ... (الكود اللاحق) ...
 ...
 _Step(timeElapsed) {
    ...
    // START_CHANGE
    // Updated HUD to display gold
    this._hud.update({
      health: playerHealth,
      day: this._dayCount,
      enemies: numEnemies,
      gold: currentPlayerGold, // Pass current gold to HUD
    });
    // END_CHANGE
  }
}
...
window.addEventListener('DOMContentLoaded', () => {
  ...
  // START_CHANGE
  // Global function to add gold via console for testing
  window.AddGold = (amount) => {
    if (_APP && _APP._entityManager) {
      const player = _APP._entityManager.Get('player');
      if (player) {
        const inventory = player.GetComponent('InventoryController');
        if (inventory) {
          inventory.AddGold(amount);
          console.log(`Added ${amount} gold. Current gold: ${inventory.GetGold()}`);
        }
      }
    }
  };
